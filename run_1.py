...
from models import PawpularMetaModel
...
def train_fn(config, meta_data):
    ...
    for epoch in range(config["train"]["max_epoch"]):
        ...
        lgbm_model.fit(features, y_truth)

        y_lgbm_train_preds = lgbm_model.predict(features)

        train_lgbm_losses.append(rmse(y_truth, y_lgbm_train_preds))

        train_nn_losses.append(running_loss / len(train_loader))
        logger.info(f'lr                  : {optimizer.param_groups[0]["lr"]}')
        logger.info(f"iteration           : {iteration}")
        logger.info(f"train nn loss          : {train_nn_losses[-1]:.8f}")
        logger.info(f"train lgbm loss           : {train_lgbm_losses[-1]:.8f}")
        ...
        y_lgbm_valid_preds = lgbm_model.predict(features)
        valid_lgbm_losses.append(rmse(y_truth, y_lgbm_valid_preds))

        mean_preds = np.mean(np.array([y_lgbm_valid_preds, preds]), axis=0)
        valid_mean_losses.append(rmse(y_truth, mean_preds))

        valid_losses.append(running_loss / len(valid_loader))
        logger.info(f"valid nn loss          : {valid_losses[-1]:.8f}")
        logger.info(f"valid lgbm loss        : {valid_lgbm_losses[-1]:.8f}")
        logger.info(f"valid mean loss        : {valid_mean_losses[-1]:.8f}")
        logger.info(f"valid nn rmse loss     : {np.sqrt(valid_losses[-1]):.8f}")
    ...
    epochs = [i + 1 for i in range(len(train_nn_losses))]
    eval_df = pd.DataFrame(index=epochs, columns=["train_eval", "valid_eval", "learning_rate"])
    eval_df["train_eval"] = train_nn_losses
    eval_df["valid_eval"] = valid_losses
    eval_df["learning_rate"] = learning_rates
    eval_df["train_lgbm_eval"] = train_lgbm_losses
    eval_df["valid_lgbm_eval"] = valid_lgbm_losses
